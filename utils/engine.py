...
def train_one_epoch(model, dataloader, loss_fn, optimizer, device):
    ...
    for images,labels in dataloader:
        ...
        batch_size=labels.size(0)       #获取lebels第0维度的长度
        total_loss+=loss.item()*batch_size
        total_correct+=(logits.argmax(dim=1)==labels).sum().item()      #预测最大值位置==真实label时就算预测正确，dim=1表示第1维即类别维度被压缩，argmax（dim=1）即每张图片最大值下标
