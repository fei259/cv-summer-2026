...
本文在 CIFAR-10 十分类任务上，使用 PyTorch 搭建两层卷积的 SimpleCNN，研究训练数据量、数据增强、Dropout 与 Weight Decay 对分类性能的影响，并修正了早期直接用测试集选择模型的流程。正式实验将官方训练集分层划分为 45,000 张训练图片与 5,000 张验证图片，由验证集选择训练轮次与候选配置，测试集仅用于最终评估。结果显示，`Dropout=0.3` 在 11,250 张训练图片的候选实验中取得 61.66% 验证准确率；扩展到全部 45,000 张并使用种子 42、123、999 与无 Dropout 基线配对复验后，测试准确率为 71.84% ± 1.22%，较基线平均提高 3.07 ± 0.74 个百分点。错误分析表明，主要困难集中在细粒度动物类别与外观相近的交通工具。受限于三个随机种子、固定训练轮数及历史测试集使用，本文结果定位为方法修正后的复验证据，而非普遍最优结论。
...
正式候选实验使用随机种子 123；100% 数据最终对照增加种子 42 和 999。每个种子均对官方训练集按类别进行分层划分：
...
- 优化器：SGD
- 学习率：0.1
- 随机种子：候选实验为 123；100% 数据最终对照为 42、123、999
- 主要评价指标：准确率与交叉熵损失
...
将 25% 数据阶段选出的 `Dropout=0.3` 扩展到全部 45,000 张训练图片，并预先设置同规模无 Dropout 基线。每个随机种子内，两组保持模型、数据划分、优化器、学习率和训练轮数一致；使用种子 42、123、999 进行配对复验（表 5）。

| 种子 | Dropout | 最佳轮次 | 训练准确率 | 验证准确率 | 测试准确率 | 测试损失 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 42 | 0.0 | 6 | 83.75% | 71.70% | 70.54% | 0.9498 |
| 42 | 0.3 | 8 | 81.30% | 73.18% | **72.82%** | 0.8484 |
| 123 | 0.0 | 5 | 79.68% | 67.08% | 66.73% | 1.0411 |
| 123 | 0.3 | 9 | 83.59% | 70.40% | **70.47%** | 1.0119 |
| 999 | 0.0 | 10 | 95.10% | 69.94% | 69.05% | 1.4933 |
| 999 | 0.3 | 10 | 84.89% | 72.26% | **72.24%** | 0.9284 |

测试准确率汇总如下（均使用样本标准差）：

- 无 Dropout：68.77% ± 1.92%。
- `Dropout=0.3`：71.84% ± 1.22%。
- 三个种子的配对提升分别为 2.28、3.74、3.19 个百分点。
- 平均配对提升为 3.07 ± 0.74 个百分点。

三个种子均获得正向提升，说明在当前 SimpleCNN、10 个 epoch、SGD 和学习率 0.1 的固定协议下，`Dropout=0.3` 的收益具有一定稳定性。由于样本仅有三个种子，仍不能据此断言该配置普遍最优或具有统计显著性。

图 1 以种子 123 为代表，展示最终 `Dropout=0.3` 模型在 100% 训练池上的训练与验证曲线；验证准确率在第 9 轮达到最高，checkpoint 机制据此保存该轮模型：
...
以种子 123 为例，同一 `Dropout=0.3` 配置从 11,250 张训练图片扩展至 45,000 张后，测试准确率由 62.58% 提高至 70.47%，增加 7.89 个百分点。该结果表明增加训练数据在这组受控条件下有明显帮助；由于跨数据规模对照只复验了一个种子，不能把该差值推广为稳定规律。

## 6. 类别错误与典型样本分析

以下错误分析使用种子 123 的 100% 数据、`Dropout=0.3` 模型，其测试准确率为 70.47%。各类别测试准确率如下（表 6）：
...
2. 100% 数据正式对照仅使用三个随机种子，虽已报告均值与样本标准差，但样本量仍不足以进行可靠的统计推断。
...
将该配置扩展到 45,000 张训练图片后，在三个随机种子上的测试准确率为 71.84% ± 1.22%，同规模无 Dropout 基线为 68.77% ± 1.92%；三个种子的配对提升均为正，平均提高 3.07 ± 0.74 个百分点。

错误分析表明，模型的主要困难集中在细粒度动物类别和外观相近的交通工具上。总体而言，增加训练数据和适度 Dropout 在当前 SimpleCNN 与训练预算下取得了正向结果，但由于随机种子数量、候选范围和历史测试集使用等限制，本报告不将其表述为普遍最优配置。

核心数据结论可归纳为三条：

1. 数据规模收益：以种子 123 为例，`Dropout=0.3` 配置从 11,250 张扩展到 45,000 张训练图片后，测试准确率由 62.58% 提高到 70.47%，提升 7.89 个百分点。
2. 正则化收益：在 45,000 张训练图片上，`Dropout=0.3` 的三种子测试准确率为 71.84% ± 1.22%，相对 68.77% ± 1.92% 的无 Dropout 基线平均提高 3.07 ± 0.74 个百分点。
